...
<tr><th>Строка</th><th>Поле</th><th>Описание</th></tr>
  <tr><td>1</td><td>tags</td><td>теги (используются только для <code>repto</code> и для идентификатора <code>ii/ok</code>)</td></tr>
...
<tr><td>5</td><td>repto</td><td>если начинается с <code>@repto:</code>, то нода проставляет тэг repto (указывает id письма, на которое отвечаем). Иначе строка относится к тексту сообщения</td></tr>
